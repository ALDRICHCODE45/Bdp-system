...
import { deactivateTarifaSchema } from "../validators/tarifasValidator";
import { toTarifaAbogadoAsuntoDto } from "../mappers/tarifaAbogadoAsuntoMapper";

export const deactivateTarifaAction = async (input: unknown) => {
  ...
  const parsed = deactivateTarifaSchema.parse(input);
