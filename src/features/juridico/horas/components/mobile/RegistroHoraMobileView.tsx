  ...
  return (
    <div className="bg-card border rounded-xl p-4 shadow-sm space-y-2.5">
      <div className="flex items-center gap-2">
        ...
        <Skeleton className="h-3.5 w-3.5 rounded-full ml-auto" />
      </div>
...
// ── Props ────────────────────────────────────────────────────────────────────
interface RegistroHoraMobileViewProps {
  data: PaginatedResult<RegistroHoraDto> | undefined;
  isLoading: boolean;
  onCreateClick: () => void;
...
export function RegistroHoraMobileView({
  data,
  isLoading,
  onCreateClick,
  onViewDetail,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b bg-background sticky top-0 z-10">
        <h1 className="text-lg font-semibold">Registro de Horas</h1>
        <div className="flex items-center gap-2">
          <Button
            size="icon"
            variant="default"
      ...
      <div className="flex-1 px-4 py-3 space-y-3">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground gap-3">
            ...
            <p className="text-xs">
              No hay registros de horas que coincidan con los filtros activos.
            </p>
          </div>
        ) : (
          data.data.map((registro) => (
            <RegistroHoraMobileCard
              key={registro.id}
              registro={registro}
      ...
      {/* ── Drawer de filtros ─────────────────────────────────────────────── */}
      <RegistroHoraMobileFiltersDrawer
        open={filtersOpen}
