...
import { EquipoJuridicoMobileCard } from "./EquipoJuridicoMobileCard";
import { EquipoJuridicoMobileFiltersDrawer } from "./EquipoJuridicoMobileFiltersDrawer";
...
// ── Props ────────────────────────────────────────────────────────────────────
interface EquipoJuridicoMobileViewProps {
  data: PaginatedResult<EquipoJuridicoDto> | undefined;
  isLoading: boolean;
  onCreateClick: () => void;
...
export function EquipoJuridicoMobileView({
  data,
  isLoading,
  onCreateClick,
  onViewDetail,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b bg-background sticky top-0 z-10">
        <h1 className="text-lg font-semibold">Equipos Jurídicos</h1>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <span className="sr-only">Nuevo equipo</span>
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 px-4 py-3 space-y-3">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground gap-3">
            ...
            <p className="text-sm font-medium">Sin equipos</p>
            <p className="text-xs">
              No hay equipos jurídicos que coincidan con los filtros activos.
            </p>
          </div>
        ) : (
          data.data.map((equipo) => (
            <EquipoJuridicoMobileCard
              key={equipo.id}
              equipo={equipo}
      ...
      {/* ── Drawer de filtros ─────────────────────────────────────────────── */}
      <EquipoJuridicoMobileFiltersDrawer
        open={filtersOpen}
