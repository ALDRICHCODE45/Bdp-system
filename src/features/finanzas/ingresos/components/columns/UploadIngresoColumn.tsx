...
interface UploadIngresoColumnProps {
  ingresoId: string;
}

export const UploadIngresoColumn = ({
  ingresoId,
}: UploadIngresoColumnProps) => {
  ...
  const loadFiles = async () => {
    setLoading(true);
    try {
      const result = await getFilesByEntityAction("INGRESO", ingresoId);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Archivos del ingreso</DialogTitle>
          <DialogDescription>
            Sube y gestiona los archivos relacionados con este ingreso (PDF,
            XML, Excel, Word)
          </DialogDescription>
        </DialogHeader>
