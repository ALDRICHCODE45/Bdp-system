...
export const IngresosColumns: ColumnDef<Ingreso>[] = [
  {
    header: "ID",
    accessorKey: "id",
    cell: ({ row }) => (
  ...
  },
  {
    ...
    accessorKey: "cliente",
    cell: ({ row }) => (
      <div className="text-sm truncate">{row.getValue("cliente")}</div>
    ),
    size: 20,
  },
  {
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="font-medium text-green-600 truncate">{formatted}</div>
      );
    },
    size: 15,
  },
...
function RowActions({ row: _row }: { row: Row<Ingreso> }) {
  return (
    <DropdownMenu>
