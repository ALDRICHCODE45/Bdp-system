...
import { useIngresoHistorial } from "../hooks/useIngresoHistorial.hook";
import {
  Timeline,
  TimelineContent,
  TimelineDate,
...
interface IngresoHistorySheetProps {
  isOpen: boolean;
  onClose: () => void;
...
export function IngresoHistorySheet({
  isOpen,
  onClose,
  ingresoConcepto,
  ingresoId,
}: IngresoHistorySheetProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = useIngresoHistorial(ingresoId, isOpen);
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side={sheetSide} className="w-full sm:w-[540px]">
        <SheetHeader>
          <SheetTitle>Historial del ingreso</SheetTitle>
          <SheetDescription>
            Información detallada de los cambios realizados para{" "}
            <span className="font-bold">{ingresoConcepto}</span>
          </SheetDescription>
        </SheetHeader>
          ...
          {!isLoading && !isError && historial && historial.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No hay historial de cambios para este ingreso
              </p>
            </div>
          )}
