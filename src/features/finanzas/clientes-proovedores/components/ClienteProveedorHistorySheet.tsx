...
interface ClienteProveedorHistorySheetProps {
  isOpen: boolean;
  onClose: () => void;
  clienteProveedorName: string;
  clienteProveedorId: string;
}

export function ClienteProveedorHistorySheet({
  isOpen,
  onClose,
  clienteProveedorName,
  clienteProveedorId,
}: ClienteProveedorHistorySheetProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = useClienteProveedorHistorial(clienteProveedorId, isOpen);
  ...
  const getFieldIcon = (campo: string) => {
    const iconMap: Record<string, React.ReactNode> = {
      ...
      tipo: <Building2 className="size-4" />,
      direccion: <MapPin className="size-4" />,
      telefono: <Phone className="size-4" />,
      email: <Mail className="size-4" />,
      contacto: <User className="size-4" />,
      numeroCuenta: <CreditCard className="size-4" />,
      clabe: <CreditCard className="size-4" />,
      banco: <CreditCard className="size-4" />,
      activo: <CheckCircle2 className="size-4" />,
      fechaRegistro: <Calendar className="size-4" />,
      notas: <FileText className="size-4" />,
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side={sheetSide} className="w-full sm:w-[540px]">
        <SheetHeader>
          <SheetTitle>Historial del cliente/proveedor</SheetTitle>
          <SheetDescription>
            Información detallada de los cambios realizados para{" "}
            <span className="font-bold">{clienteProveedorName}</span>
          </SheetDescription>
        </SheetHeader>
